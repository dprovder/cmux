...
actor DiffSidecarProcessSupervisor {
    ...
    private func cancel(requestID: String) async {
        ...
        await sendCancellation(requestID: requestID, generation: request.generation)
    }

    private func requestDidTimeOut(requestID: String, generation: UInt64) async {
        guard pending[requestID]?.generation == generation else { return }
        complete(requestID: requestID, result: .failure(SupervisorError.requestTimedOut))
        await sendCancellation(requestID: requestID, generation: generation)
    }
